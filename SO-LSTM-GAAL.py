...
def create_generator(latent_size):
    ...
    gen.add(Dense((latent_size, 1), activation='relu', kernel_initializer=keras.initializers.Identity(gain=1.0)))
...
def load_data(args):
    ...
    y = data.pop("Label")
    data_x = data.values
    data_id = data.pop("Unnamed: 0") # generic name generated by pandas because the first column hasn't name (it represents IDs) in my preproc process
    data_y = y.values
