...
def preproc_kdd(filenames):
    for filename in filenames:
        ...
        path = "Data/nsl-kdd/"
        data = pd.read_table(path + filename, sep=',', names=columns, dtype="unicode")
        data = data.iloc[1:,:]
        ...
        pd.DataFrame(data).to_csv(path + filename + "proc", header=False, index=False)

preproc_kdd(['KDDTrain', 'KDDTest'])